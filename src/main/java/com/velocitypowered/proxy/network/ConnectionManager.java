...
public final class ConnectionManager {
    ...
    public void logChannelInformation() {
        LOGGER.info("Connections will use {0} channels, {1} compression, {2} ciphers", this.transportType,
        ...
        bootstrap.bind()
                .addListener((ChannelFutureListener) future -> {
                    ...
                    if (future.isSuccess()) {
                        ...
                        LOGGER.info("Listening for GS4 query on {0}", channel.localAddress());

                        // Fire the proxy bound event after the socket is bound
                        server.getEventManager().fireAndForget(
                                new ListenerBoundEvent(address, ListenerType.QUERY));
                    } else {
                        LOGGER.error("Can't bind to {0}", bootstrap.config().localAddress(), future.cause());
                    }
                });
    }
    ...
    public void close(InetSocketAddress oldBind) {
        ...
        LOGGER.info("Closing endpoint {0}", serverChannel.localAddress());
    ...
    public void shutdown() {
        for (final Map.Entry<InetSocketAddress, Endpoint> entry : this.endpoints.entrySet()) {
            ...
            try {
                LOGGER.info("Closing endpoint {0}", address);
